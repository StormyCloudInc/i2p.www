...
Данное предложение описывает дизайн протокола, позволяющего I2P-клиенту, сервису или внешнему процессу балансировщика прозрачно управлять несколькими router'ами, хостящими один [Destination](http://localhost:63465/en/docs/specs/common-structures/#destination).

Предложение в настоящее время не определяет конкретную реализацию. Оно может быть реализовано как расширение к [I2CP](/en/docs/specs/i2cp/) или как новый протокол.
...
Для создания и управления этой конфигурацией клиенту требуется следующий новый функционал помимо того, что в настоящее время предоставляется [I2CP](/en/docs/specs/i2cp/):
