...
[Полная библиотека streaming](/spec/api/streaming/) теперь является стандартным интерфейсом. Она позволяет программировать с использованием TCP-подобных сокетов, как объясняется в [руководстве по разработке с использованием Streaming](#developing-with-the-streaming-library).
