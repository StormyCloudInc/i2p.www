...
Гарантированная доставка оставлена более высоким уровням, таким как [Streaming library (библиотека потоковой передачи данных)](/spec/api/streaming/). Приложения, которым требуется надежность, должны самостоятельно обрабатывать повторную передачу и подтверждения.
