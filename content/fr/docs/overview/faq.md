...
description: "FAQ complète I2P : aide sur le router, configuration, reseeds, confidentialité/sécurité, performance et dépannage"
slug: "faq"
lastUpdated: "2025-10"
accurateFor: "2.10.0"
type: docs
...
I2P est écrit dans le langage de programmation Java. Il a été testé sur Windows, Linux, FreeBSD et OSX. Une version Android est également disponible.

En termes d'utilisation de la mémoire, I2P est configuré pour utiliser 128 Mo de RAM par défaut. Cela est suffisant pour la navigation et l'utilisation d'IRC. Cependant, d'autres activités peuvent nécessiter une allocation de mémoire plus importante. Par exemple, si l'on souhaite faire fonctionner un router à large bande passante, participer à des torrents I2P ou héberger des services cachés à fort trafic, une quantité de mémoire plus élevée est requise.

En termes d'utilisation du processeur, I2P a été testé pour fonctionner sur des systèmes modestes tels que la gamme d'ordinateurs monocartes Raspberry Pi. Comme I2P fait un usage intensif de techniques cryptographiques, un processeur plus puissant sera mieux adapté pour gérer la charge de travail générée par I2P ainsi que les tâches liées au reste du système (c'est-à-dire le système d'exploitation, l'interface graphique, d'autres processus comme la navigation Web).

L'utilisation de Sun/Oracle Java ou OpenJDK est recommandée.

### Sur quels systèmes I2P peut-il fonctionner ? {#systems}

Oui, Java est requis pour utiliser I2P Core. Nous incluons Java dans nos installateurs faciles pour Windows, Mac OSX et Linux. Si vous utilisez l'application I2P Android, vous aurez également besoin d'un environnement d'exécution Java comme Dalvik ou ART installé dans la plupart des cas.

### L'installation de Java est-elle requise pour utiliser I2P ? {#java}

Un site I2P est un site web normal, sauf qu'il est hébergé à l'intérieur d'I2P. Les sites I2P ont des adresses qui ressemblent à des adresses internet normales, se terminant par ".i2p" d'une manière lisible par l'homme et non cryptographique, pour le bénéfice des utilisateurs. La connexion réelle à un site I2P nécessite de la cryptographie, ce qui signifie que les adresses de sites I2P sont également les longues Destinations "Base64" et les adresses "B32" plus courtes. Vous devrez peut-être effectuer une configuration supplémentaire pour naviguer correctement. La navigation sur les sites I2P nécessitera l'activation du proxy HTTP dans votre installation I2P, puis la configuration de votre navigateur pour l'utiliser. Pour plus d'informations, consultez la section "Navigateurs" ci-dessous ou le guide "Configuration du navigateur".

### Qu'est-ce qu'un "I2P Site" et comment configurer mon navigateur pour les utiliser ? {#I2P-Site}

Dans la page Pairs de votre console routeur, vous pouvez voir deux nombres - Actifs x/y. Le premier nombre est le nombre de pairs auxquels vous avez envoyé ou dont vous avez reçu un message au cours des dernières minutes. Le deuxième nombre est le nombre de pairs vus récemment, ce nombre sera toujours supérieur ou égal au premier nombre.

### Que signifient les nombres Actifs x/y dans la console du routeur ? {#active}
...
- **Partager la bande passante** - Si un router est configuré pour partager la bande passante, il acheminera davantage de trafic pour d'autres routers, ce qui aide à l'intégrer au reste du réseau, tout en améliorant les performances de sa connexion locale. Ceci peut être configuré sur la page [http://localhost:7657/config](http://localhost:7657/config).
- **Interface réseau** - Assurez-vous qu'aucune interface n'est spécifiée sur la page [http://localhost:7657/confignet](http://localhost:7657/confignet). Cela peut réduire les performances à moins que votre ordinateur soit multi-hébergé avec plusieurs adresses IP externes.
- **Protocole I2NP** - Assurez-vous que le router est configuré pour accepter les connexions sur un protocole valide pour le système d'exploitation de l'hôte et des paramètres réseau (Avancés) vides. N'entrez pas d'adresse IP dans le champ 'Nom d'hôte' de la page de configuration réseau. Le protocole I2NP que vous sélectionnez ici ne sera utilisé que si vous n'avez pas déjà une adresse joignable. La plupart des connexions sans fil Verizon 4G et 5G aux États-Unis, par exemple, bloquent l'UDP et ne peuvent pas être jointes via ce protocole. D'autres utiliseraient l'UDP de force même s'il leur est disponible. Choisissez un paramètre raisonnable parmi les protocoles I2NP listés.

### Mon routeur a très peu de pairs actifs, est-ce normal ? {#peers}

Aucun de ce matériel n'est installé par défaut. Cependant, étant donné qu'I2P est un réseau pair-à-pair, il est possible que vous rencontriez accidentellement du contenu interdit. Voici un résumé de la manière dont I2P vous évite d'être impliqué inutilement dans des violations de vos convictions.
...
### Je suis opposé à certains types de contenus. Comment puis-je éviter de les distribuer, de les stocker ou d'y accéder ? {#badcontent}

Oui, de loin le moyen le plus simple et le plus courant est de bloquer le bootstrap, ou les serveurs "Reseed". Bloquer complètement tout le trafic obfusqué fonctionnerait également (bien que cela casserait beaucoup, beaucoup d'autres choses qui ne sont pas I2P et la plupart ne sont pas prêts à aller aussi loin). Dans le cas du blocage du reseed, il existe un bundle reseed sur Github, le bloquer bloquera également Github. Vous pouvez effectuer un reseed via un proxy (beaucoup peuvent être trouvés sur Internet si vous ne voulez pas utiliser Tor) ou partager des bundles reseed entre amis de manière hors ligne.

### Est-il possible de bloquer I2P ? {#blocking}

Souvent, cette erreur se produit avec tout logiciel Java activé pour le réseau sur certains systèmes configurés pour utiliser IPv6 par défaut. Il existe plusieurs façons de résoudre ce problème :
...
- Recherchez les lignes suivantes dans `wrapper.config` :
  ...
  ```
  Si ces lignes sont présentes, décommentez-les en supprimant les "#". Si elles ne sont pas présentes, ajoutez-les sans les "#".

Une autre option serait de supprimer le `::1` de `~/.i2p/clients.config`

**ATTENTION** : Pour que les modifications apportées à `wrapper.config` prennent effet, vous devez arrêter complètement le router et le wrapper. Cliquer sur *Redémarrer* dans votre console du routeur ne relira PAS ce fichier ! Vous devez cliquer sur *Arrêter*, attendre 11 minutes, puis démarrer I2P.
...
Si vous considérez tous les I2P Sites qui ont été créés, oui, la plupart sont hors ligne. Les personnes et les I2P Sites vont et viennent. Une bonne façon de débuter sur I2P est de consulter une liste d'I2P Sites actuellement en ligne. [identiguy.i2p](http://identiguy.i2p) suit les I2P Sites actifs.

### La plupart des sites I2P au sein d'I2P sont hors ligne ? {#down}

Le wrapper de service Java Tanuki que nous utilisons ouvre ce port — lié à localhost — afin de communiquer avec le logiciel s'exécutant dans la JVM. Lorsque la JVM est lancée, une clé lui est fournie pour qu'elle puisse se connecter au wrapper. Une fois que la JVM a établi sa connexion au wrapper, ce dernier refuse toute connexion supplémentaire.

Plus d'informations sont disponibles dans la [documentation du wrapper](http://wrapper.tanukisoftware.com/doc/english/prop-port.html).

### Pourquoi I2P écoute-t-il sur le port 32000 ? {#port32000}

La configuration du proxy pour différents navigateurs se trouve sur une page séparée avec des captures d'écran. Des configurations plus avancées avec des outils externes, tels que le plugin de navigateur FoxyProxy ou le serveur proxy Privoxy, sont possibles mais pourraient introduire des fuites dans votre configuration.
...
```
### Comment se connecter à IRC dans I2P ? {#irc}

La méthode la plus simple consiste à cliquer sur le lien [i2ptunnel](http://127.0.0.1:7657/i2ptunnel/) dans la console du routeur et à créer un nouveau 'Tunnel Serveur'. Vous pouvez servir du contenu dynamique en définissant la destination du tunnel vers le port d'un serveur web existant, tel que Tomcat ou Jetty. Vous pouvez également servir du contenu statique. Pour cela, définissez la destination du tunnel sur : `0.0.0.0 port 7659` et placez le contenu dans le répertoire `~/.i2p/eepsite/docroot/`. (Sur les systèmes non-Linux, cela peut se trouver à un emplacement différent. Vérifiez la console du routeur.) Le logiciel 'eepsite' est fourni dans le paquet d'installation d'I2P et est configuré pour démarrer automatiquement au lancement d'I2P. Le site par défaut ainsi créé est accessible à l'adresse http://127.0.0.1:7658. Cependant, votre 'eepsite' est également accessible aux autres via votre fichier de clés eepsite, situé à : `~/.i2p/eepsite/i2p/eepsite.keys`. Pour en savoir plus, consultez le fichier readme à : `~/.i2p/eepsite/README.txt`.

### Comment configurer mon propre site I2P ? {#myI2P-Site}

Cela dépend de votre adversaire et de votre modèle de menace. Si vous vous inquiétez uniquement des violations « de confidentialité » des entreprises, des criminels typiques et de la censure, alors ce n'est pas vraiment dangereux. Les forces de l'ordre vous trouveront probablement de toute façon s'ils le veulent vraiment. Héberger uniquement lorsque vous avez un navigateur domestique normal (Internet) en cours d'exécution rendra vraiment difficile de savoir qui héberge cette partie. Veuillez considérer l'hébergement de votre site I2P comme l'hébergement de tout autre service - c'est aussi dangereux - ou sûr - que vous le configurez et le gérez vous-même.

Note : Il existe déjà un moyen de séparer l'hébergement d'un service i2p (destination) du routeur i2p. Si vous [comprenez comment](/docs/overview/tech-intro#i2pservices) cela fonctionne, vous pouvez simplement configurer une machine séparée comme serveur pour le site web (ou service) qui sera accessible publiquement et le transférer vers le serveur web via un tunnel SSH [très] sécurisé ou utiliser un système de fichiers partagé et sécurisé.

### Si j'héberge un site web sur I2P chez moi, ne contenant que du HTML et CSS, est-ce dangereux ? {#hosting}

L'application Carnet d'adresses I2P associe des noms lisibles par l'homme à des destinations à long terme, liées à des services, ce qui la rend plus proche d'un fichier hosts ou d'une liste de contacts que d'une base de données réseau ou d'un service DNS. Elle fonctionne également en mode local d'abord : il n'existe pas d'espace de noms global reconnu, c'est vous qui décidez à quoi correspond finalement un domaine .i2p donné. La solution intermédiaire est un système appelé « Service de Redirection » (Jump Service) qui fournit un nom lisible en vous redirigeant vers une page où il vous sera demandé « Autorisez-vous le routeur I2P à appeler $SITE_CRYPTO_KEY du nom $SITE_NAME.i2p » ou quelque chose dans ce genre. Une fois l'entrée enregistrée dans votre carnet d'adresses, vous pouvez générer vos propres URL de redirection pour aider à partager le site avec d'autres.

### Comment I2P trouve-t-il les sites web ".i2p" ? {#addresses}

Vous ne pouvez pas ajouter une adresse sans connaître au moins le base32 ou le base64 du site que vous souhaitez visiter. Le "nom d'hôte" lisible par l'humain n'est qu'un alias pour l'adresse cryptographique, qui correspond au base32 ou au base64. Sans l'adresse cryptographique, il n'y a aucun moyen d'accéder à un site I2P, c'est voulu par conception. La distribution de l'adresse aux personnes qui ne la connaissent pas encore relève généralement de la responsabilité du fournisseur de service Jump. La visite d'un site I2P qui est inconnu déclenchera l'utilisation d'un service Jump. stats.i2p est le service Jump le plus fiable.

Si vous hébergez un site via i2ptunnel, celui-ci n'aura pas encore d'enregistrement auprès d'un service de redirection (jump service). Pour lui attribuer une URL localement, visitez la page de configuration et cliquez sur le bouton « Add to Local Address Book ». Ensuite, rendez-vous sur http://127.0.0.1:7657/dns pour rechercher l'URL addresshelper et la partager.
...
Note : Depuis la version 0.7.8, les nouvelles installations n'utilisent pas le port 8887 ; un port aléatoire entre 9000 et 31000 est sélectionné lors de la première exécution du programme. Le port sélectionné est affiché sur la [page de configuration](http://127.0.0.1:7657/confignet) du routeur.
...
- TCP depuis des ports élevés aléatoires vers des ports TCP distants arbitraires
- UDP sortant sur le port 123, permettant les réponses. Ceci est nécessaire pour la synchronisation temporelle interne d'I2P (via SNTP - interrogation d'un hôte SNTP aléatoire dans pool.ntp.org ou un autre serveur que vous spécifiez)
...
**Quels sont les bons liens d'abonnement au carnet d'adresses ?**
...
Pour des raisons de sécurité, la console d'administration du routeur écoute par défaut uniquement les connexions sur l'interface locale.

Il existe deux méthodes pour accéder à la console à distance :

1. Tunnel SSH
2. Configuration de votre console pour qu'elle soit accessible sur une adresse IP publique avec un nom d'utilisateur et un mot de passe

Ceux-ci sont détaillés ci-dessous :

**Méthode 1 : Tunnel SSH**

Si vous utilisez un système d'exploitation de type Unix, il s'agit de la méthode la plus simple pour accéder à distance à votre console I2P. (Remarque : un logiciel serveur SSH est disponible pour les systèmes Windows, par exemple [https://github.com/PowerShell/Win32-OpenSSH](https://github.com/PowerShell/Win32-OpenSSH))

Une fois que vous avez configuré l'accès SSH à votre système, le flag '-L' est passé à SSH avec les arguments appropriés - par exemple :

```
ssh -L 7657:localhost:7657 (System_IP)
```
où '(System_IP)' est remplacé par l'adresse IP de votre système. Cette commande transfère le port 7657 (le numéro avant le premier deux-points) vers le port 7657 du système distant (tel que spécifié par la chaîne 'localhost' entre les premier et second deux-points) (le numéro après le second deux-points). Votre console I2P distante sera maintenant accessible sur votre système local à l'adresse 'http://localhost:7657' et restera disponible tant que votre session SSH sera active.

Si vous souhaitez démarrer une session SSH sans lancer de shell sur le système distant, vous pouvez ajouter le drapeau '-N' :
...
2. Allez sur [http://localhost:7657/configui](http://localhost:7657/configui) et ajoutez un nom d'utilisateur et un mot de passe pour la console si vous le souhaitez - L'ajout d'un nom d'utilisateur et d'un mot de passe est fortement recommandé pour sécuriser votre console I2P contre toute manipulation, ce qui pourrait entraîner une désanonymisation.

3. Allez sur [http://localhost:7657/index](http://localhost:7657/index) et cliquez sur "Graceful restart", ce qui redémarre la JVM et recharge les applications clientes

Après le démarrage, vous devriez maintenant pouvoir accéder à votre console à distance. Chargez la console du routeur à l'adresse `http://(System_IP):7657` et vous serez invité à saisir le nom d'utilisateur et le mot de passe que vous avez spécifiés à l'étape 2 ci-dessus si votre navigateur prend en charge la fenêtre d'authentification.

NOTE : Vous pouvez spécifier 0.0.0.0 dans la configuration ci-dessus. Ceci spécifie une interface, pas un réseau ou un masque réseau. 0.0.0.0 signifie "lier à toutes les interfaces", de sorte qu'il peut être accessible sur 127.0.0.1:7657 ainsi que sur n'importe quelle IP LAN/WAN. Soyez prudent lors de l'utilisation de cette option car la console sera disponible sur TOUTES les adresses configurées sur votre système.
...
### Il manque beaucoup d'hôtes dans mon carnet d'adresses. Quels sont les bons liens d'abonnement ? {#subscriptions}

Le proxy SOCKS est fonctionnel depuis la version 0.7.1. SOCKS 4/4a/5 sont pris en charge. I2P ne dispose pas d'outproxy SOCKS, il est donc limité à une utilisation au sein d'I2P uniquement.

De nombreuses applications divulguent des informations sensibles qui pourraient vous identifier sur Internet et c'est un risque dont vous devez être conscient lors de l'utilisation du proxy SOCKS I2P. I2P ne filtre que les données de connexion, mais si le programme que vous avez l'intention d'exécuter envoie ces informations en tant que contenu, I2P n'a aucun moyen de protéger votre anonymat. Par exemple, certaines applications de messagerie enverront l'adresse IP de la machine sur laquelle elles s'exécutent à un serveur de messagerie. Nous recommandons des outils ou applications spécifiques à I2P (tels que [I2PSnark](http://localhost:7657/i2psnark/) pour les torrents), ou des applications connues pour être sûres à utiliser avec I2P qui incluent des plugins populaires disponibles sur [Firefox](https://www.mozilla.org/).

### Comment puis-je accéder à la console web depuis mes autres machines ou la protéger par mot de passe ? {#remote_webconsole}

Il existe des services appelés Outproxies qui font le pont entre I2P et Internet, comme les nœuds de sortie Tor. La fonctionnalité outproxy par défaut pour HTTP et HTTPS est fournie par `exit.stormycloud.i2p` et est gérée par StormyCloud Inc. Elle est configurée dans le proxy HTTP. De plus, pour aider à protéger l'anonymat, I2P ne vous permet pas de faire des connexions anonymes vers Internet classique par défaut. Veuillez consulter la page [Socks Outproxy](/docs/api/socks#outproxy) pour plus d'informations.
...
Vérifiez d'abord la page [http://127.0.0.1:7657/netdb](http://127.0.0.1:7657/netdb) dans la Console du Routeur – votre base de données réseau. Si vous ne voyez aucun routeur listé depuis I2P mais que la console indique que vous devriez être derrière un pare-feu, alors vous ne pouvez probablement pas vous connecter aux serveurs de reseed. Si vous voyez d'autres routeurs I2P listés, essayez de réduire le nombre de connexions maximales [http://127.0.0.1:7657/config](http://127.0.0.1:7657/config) peut-être que votre routeur ne peut pas gérer autant de connexions.

### Est-il possible d'utiliser I2P comme proxy SOCKS ? {#socks}

Dans des circonstances normales, I2P vous connectera automatiquement au réseau en utilisant nos liens d'amorçage. Si une connexion Internet perturbée empêche l'amorçage depuis les serveurs reseed, une méthode simple pour amorcer consiste à utiliser le navigateur Tor (qui ouvre localhost par défaut), ce qui fonctionne très bien avec [http://127.0.0.1:7657/configreseed](http://127.0.0.1:7657/configreseed). Il est également possible de réamorcer un router I2P manuellement.

Lorsque vous utilisez le navigateur Tor pour effectuer un reseed, vous pouvez sélectionner plusieurs URL à la fois et continuer. Bien que la valeur par défaut qui est de 2 (parmi les multiples urls) fonctionne également, ce sera lent.

---

## Privacy-Safety

### Comment accéder à IRC, BitTorrent ou d'autres services sur l'Internet classique ? {#proxy_other}
...
Le trafic I2P ressemble généralement à du trafic UDP, et pas beaucoup plus – et le faire ressembler à pas beaucoup plus est un objectif. Il prend également en charge TCP. Avec quelques efforts, l'analyse passive du trafic peut être capable de classifier le trafic comme étant « I2P », mais nous espérons que le développement continu de l'obscurcissement du trafic réduira cela davantage. Même une couche d'obscurcissement de protocole assez simple comme obfs4 empêchera les censeurs de bloquer I2P (c'est un objectif que I2P déploie).

### Mon routeur fonctionne depuis plusieurs minutes et n'a aucune connexion ou très peu de connexions {#reseed}

Cela dépend de votre modèle de menace personnel. Pour la plupart des gens, I2P est bien plus sûr que de n'utiliser aucune protection. Certains autres réseaux (comme Tor, mixminion/mixmaster) sont probablement plus sûrs contre certains adversaires. Par exemple, le trafic I2P n'utilise pas TLS/SSL, il n'a donc pas les problèmes du "maillon le plus faible" que Tor a. I2P a été utilisé par beaucoup de personnes en Syrie lors du "printemps arabe", et récemment le projet a connu une croissance plus importante dans les installations linguistiques plus petites d'I2P au Proche et Moyen-Orient. Le point le plus important à noter ici est qu'I2P est une technologie et vous avez besoin d'un guide pratique pour améliorer votre vie privée/anonymat sur Internet. Vérifiez également votre navigateur ou importez le moteur de recherche d'empreintes numériques pour bloquer les attaques par empreinte numérique avec un ensemble de données très volumineux (c'est-à-dire : longues traînes typiques / structure de données diverse très précise) concernant de nombreux éléments d'environnement et n'utilisez pas de VPN pour réduire tous les risques qui en découlent comme le comportement du cache TLS propre et la construction technique de l'entreprise fournisseur qui peut être piratée plus facilement qu'un système de bureau personnel. L'utilisation d'un navigateur Tor V-Browser isolé avec ses excellentes protections anti-empreinte numérique et une protection appguard globale à vie autorisant uniquement les communications système nécessaires et une utilisation finale en VM avec des scripts de désactivation anti-espion et un live-CD pour éliminer tout "risque presque permanent possible" et réduire tous les risques par une probabilité décroissante sont une bonne option dans un réseau public et un modèle de risque individuel élevé et pourraient être le mieux que vous puissiez faire avec cet objectif pour l'utilisation d'I2P.

### Comment réamorcer manuellement ? {#manual_reseed}

Oui, pour les autres nœuds I2P qui connaissent votre router. Nous utilisons cela pour nous connecter au reste du réseau I2P. Les adresses sont physiquement situées dans des objets "routerInfos (clé,valeur)", soit récupérés à distance, soit reçus d'un pair. Les "routerInfos" contiennent certaines informations (certaines ajoutées de manière opportuniste et optionnelle), "publiées par le pair", concernant le router lui-même pour l'amorçage. Aucune donnée concernant les clients ne se trouve dans cet objet. En regardant de plus près sous le capot, vous verrez que tout le monde est compté avec le type le plus récent de création d'identifiants appelé "Hachages SHA-256 (bas=Hachage positif(-clé), haut=Hachage négatif(+clé))". Le réseau I2P dispose de sa propre base de données de routerInfos créés lors du téléchargement et de l'indexation, mais cela dépend profondément de la réalisation des tables clé/valeur et de la topologie du réseau ainsi que de l'état de charge / état de bande passante et des probabilités de routage pour les stockages dans les composants de la base de données.

### Is using an outproxy safe? {#proxy_safe}

Cela dépend de votre définition de « sûr ». Les outproxies sont excellents quand ils fonctionnent, mais malheureusement ils sont gérés bénévolement par des personnes qui peuvent perdre intérêt ou ne pas avoir les ressources pour les maintenir 24h/24 et 7j/7 – veuillez noter que vous pouvez rencontrer des périodes durant lesquelles les services sont indisponibles, interrompus ou peu fiables, et nous ne sommes pas associés à ce service et n'avons aucune influence sur celui-ci.
...
### Mon routeur est-il un "nœud de sortie" (outproxy) vers l'Internet classique ? Je ne veux pas qu'il le soit. {#exit}
...
Le proxy vers les sites Internet (eepsites qui sont dirigés vers Internet) est fourni en tant que service aux utilisateurs I2P par des fournisseurs non bloquants. Ce service n'est pas l'objectif principal du développement d'I2P et est fourni sur une base volontaire. Les eepsites hébergés sur I2P devraient toujours fonctionner sans outproxy. Les outproxies sont pratiques mais ils ne sont pas parfaits par conception ni une partie importante du projet. Sachez qu'ils peuvent ne pas être en mesure de fournir le service de haute qualité que d'autres services d'I2P peuvent offrir.

### Est-il sûr d'utiliser I2P ? {#safe}

Le proxy HTTP par défaut prend en charge uniquement le outproxying HTTP et HTTPS.
...
Tout d'abord, assurez-vous d'avoir la dernière version de chaque composant lié à I2P – les anciennes versions contenaient des sections de code inutilement gourmandes en CPU. Il existe également un [journal de performance](/about/performance) qui documente certaines des améliorations des performances d'I2P au fil du temps.
...
### Qu'en est-il des attaques de "dés-anonymisation" ? {#deanon}

I2P dispose de différentes protections qui ajoutent un routage supplémentaire et des couches de chiffrement additionnelles. Il fait également rebondir le trafic à travers d'autres pairs (Tunnels) qui ont leur propre vitesse et qualité, certains sont lents, d'autres rapides. Cela s'accumule en beaucoup de surcharge et de trafic à différents rythmes dans différentes directions. Par conception, toutes ces choses le rendront plus lent qu'une connexion directe sur internet, mais beaucoup plus anonyme et toujours assez rapide pour la plupart des usages.

Ci-dessous se trouve un exemple présenté avec une explication pour aider à fournir du contexte concernant les considérations de latence et de bande passante lors de l'utilisation d'I2P.
...
Deuxièmement, nous pouvons considérer la bande passante disponible. Celle-ci est déterminée par le lien le plus lent entre les sauts du client au serveur ainsi que lorsque le trafic est transmis par le serveur vers le client. Pour le trafic allant du client vers le serveur, nous constatons que la bande passante disponible dans notre exemple entre les sauts 'R' et 'X' ainsi qu'entre les sauts 'X' et 'Y' est de 32 Ko/s. Malgré une bande passante disponible plus élevée entre les autres sauts, ces sauts agiront comme un goulot d'étranglement et limiteront la bande passante maximale disponible pour le trafic de 'A' à 'B' à 32 Ko/s. De même, en traçant le chemin du serveur vers le client, on constate qu'il y a une bande passante maximale de 64 Ko/s - entre les sauts 'Z_1' et 'Y_1', 'Y_1' et 'X_1' et 'Q_1' et 'P_1'.
...
Nous recommandons également de définir une quantité suffisante de bande passante partagée - cela permet aux tunnels participants d'être acheminés via votre routeur I2P. Autoriser le trafic participant maintient votre routeur bien intégré dans le réseau et améliore vos vitesses de transfert.

I2P est un projet en cours de développement. De nombreuses améliorations et corrections sont constamment mises en œuvre et, de manière générale, utiliser la dernière version contribuera à améliorer vos performances. Si ce n'est pas déjà fait, installez la dernière version.

### I think I found a bug, where can I report it? {#bug}

Vous pouvez signaler tout bug ou problème rencontré sur notre système de suivi de bugs, accessible à la fois via Internet classique et I2P. Nous avons également un forum de discussion, disponible sur I2P et Internet classique. Vous pouvez aussi rejoindre notre canal IRC : soit via notre réseau IRC, IRC2P, soit sur Freenode.
...
- **Coller les journaux :** Vous pouvez coller tout journal intéressant sur un service de collage tel que les services Internet non-privés listés sur le [Wiki PrivateBin](https://github.com/PrivateBin/PrivateBin/wiki/PrivateBin-Directory), ou un service de collage I2P tel que cette [instance PrivateBin](http://paste.crypthost.i2p) ou ce [service de collage sans Javascript](http://pasta-nojs.i2p) et faire un suivi sur IRC dans #i2p
- **IRC :** Rejoignez #i2p-dev pour discuter avec les développeurs sur IRC

Veuillez inclure les informations pertinentes de la page des journaux du router disponible à l'adresse : [http://127.0.0.1:7657/logs](http://127.0.0.1:7657/logs). Nous vous demandons de partager tout le texte de la section « I2P Version and Running Environment » ainsi que toutes les erreurs ou avertissements affichés dans les différents journaux présents sur la page.

---

### Je ne peux pas accéder aux sites Internet classiques via I2P. {#outproxy}

Super ! Rejoignez-nous sur IRC :
