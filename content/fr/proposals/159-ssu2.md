...
Cette proposition décrit un protocole d'accord de clés authentifié pour améliorer la résistance de [SSU](/en/docs/transport/ssu/) à diverses formes d'identification automatisée et d'attaques.
...
Comme avec les autres transports I2P, SSU2 est défini pour le transport point-à-point (routeur-à-routeur) des messages I2NP. Ce n'est pas un canal de données à usage général. Comme [SSU](/en/docs/transport/ssu/), il fournit également deux services supplémentaires : le relais pour la traversée NAT et les tests de pairs pour la détermination de l'accessibilité entrante. Il fournit également un troisième service, absent dans SSU, pour la migration de connexion lorsqu'un pair change d'IP ou de port.
...
- Support SSU 1 et 2 sur un seul port, détection automatique, et publié comme un seul
  "transport" (c'est-à-dire RouterAddress) dans la [NetDB](/en/docs/how/network-database/).
...
- Utiliser une poignée de main à 3 messages et un seul aller-retour, comme dans [NTCP2](/en/docs/specs/ntcp2/).
  Supprimer le délai d'attente des messages de données qui fait de
  [SSU](/en/docs/transport/ssu/) effectivement une poignée de main à deux allers-retours.
...
- Résistance DPI à toute épreuve... il s'agirait de transports pluggables,
  [Proposition 109](/en/proposals/109-pt-transport/).

- Un transport basé sur TLS (ou ressemblant à HTTPS)... qui serait [Proposition 104](/en/proposals/104-tls-transport/).
...
Cette proposition tente de fournir les cinq exigences basées sur le protocole Station-To-Station (STS). Notez que ce protocole est également la base du protocole [SSU](/en/docs/transport/ssu/).
...
* Modèles de menace : À partir de NTCP2 [NTCP2](/en/docs/specs/ntcp2/), avec des menaces supplémentaires significatives
...
* Choix cryptographiques : De [NTCP2](/en/docs/specs/ntcp2/).

* Handshake : Noise XK de [NTCP2](/en/docs/specs/ntcp2/) et [NOISE](https://noiseprotocol.org/noise.html). Des simplifications importantes
  de NTCP2 sont possibles grâce à l'encapsulation (limites de messages inhérentes)
  fournie par UDP.

* Obscurcissement de clé éphémère de poignée de main : Adapté de [NTCP2](/en/docs/specs/ntcp2/)
  mais utilisant ChaCha20 de [ECIES](/en/docs/specs/ecies/) au lieu d'AES.
...
* Obfuscation d'en-tête de paquet : Adapté de [NTCP2](/en/docs/specs/ntcp2/)
  mais utilisant ChaCha20 de [ECIES](/en/docs/specs/ecies/) au lieu d'AES.
...
* En-têtes utilisés comme données associées AEAD comme dans [ECIES](/en/docs/specs/ecies/).
...
* Messages : Adapté de [SSU](/en/docs/transport/ssu/)

* Fragmentation I2NP : Adapté de [SSU](/en/docs/transport/ssu/)

* Relais et Test de Pairs : Adapté de [SSU](/en/docs/transport/ssu/)

* Signatures des données Relay et Peer Test : À partir de la spécification des structures communes [Common](/en/docs/specs/common-structures/)

* Format de bloc : Depuis [NTCP2](/en/docs/specs/ntcp2/) et [ECIES](/en/docs/specs/ecies/).

* Rembourrage et options : À partir de [NTCP2](/en/docs/specs/ntcp2/) et [ECIES](/en/docs/specs/ecies/).
...
Ceci utilise le même format de bloc que défini dans les spécifications [NTCP2](/en/docs/specs/ntcp2/) et [ECIES](/en/docs/specs/ecies/). Les types de blocs individuels sont définis différemment. Le terme équivalent dans QUIC [RFC 9000](https://datatracker.ietf.org/doc/html/rfc9000) est "frames".
...
Il y a un ou plusieurs blocs dans la charge utile chiffrée. Un bloc est un format simple Tag-Length-Value (TLV). Chaque bloc contient un identifiant d'un octet, une longueur de deux octets, et zéro ou plusieurs octets de données. Ce format est identique à celui de [NTCP2](/en/docs/specs/ntcp2/) et [ECIES](/en/docs/specs/ecies/), cependant les définitions de blocs sont différentes.
...
Cela utilise les mêmes 9 octets pour l'en-tête I2NP que dans [NTCP2](/en/docs/specs/ntcp2/) (type, identifiant de message, expiration courte).
...
- La longueur du padding doit être déterminée soit au cas par cas pour chaque message avec des estimations de la distribution de longueur, soit des délais aléatoires doivent être ajoutés. Ces contre-mesures doivent être incluses pour résister à la DPI, car les tailles de messages révéleraient sinon que le trafic I2P est transporté par le protocole de transport. Le schéma exact de padding constitue un domaine de travail futur, l'Annexe A de [NTCP2](/en/docs/specs/ntcp2/) fournit plus d'informations sur le sujet.
...
Toutes les fonctionnalités du relais SSU 1 sont prises en charge, y compris celles documentées dans la [Proposition 158](/en/proposals/158-ipv6-transport-enhancements/) et supportées depuis la version 0.9.50. Les introductions IPv4 et IPv6 sont prises en charge. Une demande de relais peut être envoyée via une session IPv4 pour une introduction IPv6, et une demande de relais peut être envoyée via une session IPv6 pour une introduction IPv4.
...
Les propriétés d'adresse suivantes peuvent être publiées, inchangées par rapport à SSU 1, y compris les modifications de la [Proposition 158](/en/proposals/158-ipv6-transport-enhancements/) prises en charge à partir de l'API 0.9.50 :
