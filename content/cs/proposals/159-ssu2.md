...
Tento návrh popisuje protokol pro ověřené dohodnutí klíčů za účelem zlepšení odolnosti [SSU](/en/docs/transport/ssu/) proti různým formám automatické identifikace a útoků.
...
Stejně jako ostatní I2P transporty je SSU2 definován pro point-to-point (router-to-router) přenos I2NP zpráv. Není to obecný datový kanál. Podobně jako [SSU](/en/docs/transport/ssu/) také poskytuje dvě dodatečné služby: Relaying pro průchod NAT a Peer Testing pro určení dostupnosti příchozích spojení. Poskytuje také třetí službu, která není v SSU, pro migraci spojení, když peer změní IP nebo port.
...
- Podporuje SSU 1 i 2 na jednom portu, automatická detekce a publikováno jako jeden
  "transport" (tj. RouterAddress) v [NetDB](/en/docs/how/network-database/).
...
- Použijte 3-zprávu, jednokolový handshake, jako v [NTCP2](/en/docs/specs/ntcp2/).
  Odstraňte zpoždění čekání na datové zprávy, které dělá z
  [SSU](/en/docs/transport/ssu/) efektivně dvoukolový handshake.
...
- Neprůstřelná odolnost vůči DPI... to by byly pluggable transports,
  [Návrh 109](/en/proposals/109-pt-transport/).

- TLS-založený (nebo HTTPS-podobný) transport... to by byl [Proposal 104](/en/proposals/104-tls-transport/).
...
Současný návrh se pokouší poskytnout všech pět požadavků založených na protokolu Station-To-Station (STS). Upozorňujeme, že tento protokol je také základem pro protokol [SSU](/en/docs/transport/ssu/).
...
* Modely hrozeb: Z NTCP2 [NTCP2](/en/docs/specs/ntcp2/), s významnými dodatečnými hrozbami
...
* Kryptografické volby: Z [NTCP2](/en/docs/specs/ntcp2/).

* Handshake: Noise XK z [NTCP2](/en/docs/specs/ntcp2/) a [NOISE](https://noiseprotocol.org/noise.html). Značná zjednodušení
  NTCP2 jsou možná díky enkapsulaci (inherentní hranice zpráv)
  poskytované protokolem UDP.

* Handshake ephemeral key obfuscation: Upraveno z [NTCP2](/en/docs/specs/ntcp2/)
  ale s použitím ChaCha20 z [ECIES](/en/docs/specs/ecies/) místo AES.
...
* Obfuskace záhlaví paketů: Adaptováno z [NTCP2](/en/docs/specs/ntcp2/)
  ale používá ChaCha20 z [ECIES](/en/docs/specs/ecies/) místo AES.
...
* Hlavičky používané jako AEAD associated data jako v [ECIES](/en/docs/specs/ecies/).
...
* Zprávy: Adaptováno z [SSU](/en/docs/transport/ssu/)

* I2NP fragmentace: Adaptováno z [SSU](/en/docs/transport/ssu/)

* Relay a Peer Testing: Adaptováno z [SSU](/en/docs/transport/ssu/)

* Podpisy dat Relay a Peer Test: Ze specifikace společných struktur [Common](/en/docs/specs/common-structures/)

* Formát bloku: Z [NTCP2](/en/docs/specs/ntcp2/) a [ECIES](/en/docs/specs/ecies/).

* Padding a možnosti: Z [NTCP2](/en/docs/specs/ntcp2/) a [ECIES](/en/docs/specs/ecies/).
...
Tento návrh poskytuje požadavky založené na Noise Protocol Framework [NOISE](https://noiseprotocol.org/noise.html) (Revize 33, 2017-10-04). Noise má podobné vlastnosti jako protokol Station-To-Station (STS), který je základem pro protokol [SSU](/en/docs/transport/ssu/). V terminologii Noise je Alice iniciátor a Bob respondér.
...
Toto používá stejný formát bloku, jak je definován ve specifikacích [NTCP2](/en/docs/specs/ntcp2/) a [ECIES](/en/docs/specs/ecies/). Jednotlivé typy bloků jsou definovány odlišně. Ekvivalentní termín v QUIC [RFC 9000](https://datatracker.ietf.org/doc/html/rfc9000) je "frames".
...
V šifrovaném payloadu je jeden nebo více bloků. Blok je jednoduchý formát Tag-Length-Value (TLV). Každý blok obsahuje jednobajtový identifikátor, dvoubytovou délku a nula nebo více bajtů dat. Tento formát je identický s formátem v [NTCP2](/en/docs/specs/ntcp2/) a [ECIES](/en/docs/specs/ecies/), nicméně definice bloků se liší.
...
Používá stejných 9 bytů pro I2NP hlavičku jako v [NTCP2](/en/docs/specs/ntcp2/) (typ, message id, krátké vypršení).
...
Toto používá stejných 9 bytů pro I2NP hlavičku jako v [NTCP2](/en/docs/specs/ntcp2/) (typ, id zprávy, krátká expirace).
...
- Délka paddingu má být buď rozhodována na základě jednotlivých zpráv a
  ...
  dokumentu [NTCP2](/en/docs/specs/ntcp2/) poskytuje více informací k tomuto tématu.
...
Všechny funkce SSU 1 relay jsou podporovány, včetně těch zdokumentovaných v [Proposal 158](/en/proposals/158-ipv6-transport-enhancements/) a podporovaných od verze 0.9.50. IPv4 a IPv6 introdukce jsou podporovány. Relay Request může být odeslán přes IPv4 relaci pro IPv6 introdukci a Relay Request může být odeslán přes IPv6 relaci pro IPv4 introdukci.
...
Následující vlastnosti adres mohou být publikovány, nezměněné od SSU 1, včetně změn v [Proposal 158](/en/proposals/158-ipv6-transport-enhancements/) podporovaných od API 0.9.50:
