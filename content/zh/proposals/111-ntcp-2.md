...
提案阶段已结束。请参阅[规范](/en/docs/specs/ntcp2/)获取官方规格说明。本提案仍可作为背景信息参考。

## 概述

此提案描述了一个认证密钥协商协议，用于提高 [NTCP](/en/docs/transport/ntcp/) 对各种自动化识别和攻击的抗性。
...
[NTCP](/en/docs/transport/ntcp/) 数据在第一条消息之后被加密（并且第一条消息看起来是随机数据），从而通过"有效载荷分析"防止协议识别。但它仍然容易受到通过"流量分析"进行的协议识别攻击。这是因为前4条消息（即握手过程）具有固定长度（288、304、448和48字节）。

通过向每条消息添加随机数量的随机数据，我们可以让攻击变得更加困难。

作者们承认，标准安全实践会建议使用现有协议（如TLS），但这是[Prop104](/en/proposals/104/)，它本身也存在问题。在适当的地方，已添加"未来工作"段落来说明缺失的功能或讨论主题。

## 设计目标

- 在单个端口上支持 NTCP 1 和 2，自动检测，并在 [NetDB](/en/docs/how/network-database/) 中作为单个"传输"（即 RouterAddress）发布。
...
- 如果可能，将4消息、两次往返握手减少为3消息、一次往返握手，如 [SSU](/en/docs/transport/ssu/) 中所述。这需要将 Bob 在消息4中的签名移动到消息2中。在十年前的邮件/状态/会议存档中研究使用4消息的原因。
...
- 基于 TLS 的（或类似 HTTPS 的）传输协议... 这将是 [Prop104](/en/proposals/104/)。
...
本提案尝试基于 Station-To-Station (STS) 协议来提供所有五个要求。请注意，该协议也是 [SSU](/en/docs/transport/ssu/) 协议的基础。
...
此提案基于 Noise Protocol Framework [NOISE](https://noiseprotocol.org/noise.html)（修订版 33，2017-10-04）提供要求。Noise 具有与 Station-To-Station 协议类似的特性，该协议是 [SSU](/en/docs/transport/ssu/) 协议的基础。在 Noise 术语中，Alice 是发起者，Bob 是响应者。
