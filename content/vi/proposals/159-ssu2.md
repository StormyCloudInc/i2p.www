...
Đề xuất này mô tả một giao thức thỏa thuận khóa được xác thực nhằm cải thiện khả năng chống lại các hình thức tự động hóa nhận dạng và tấn công khác nhau của [SSU](/en/docs/transport/ssu/).
...
- Hỗ trợ SSU 1 và 2 trên một cổng duy nhất, tự động phát hiện, và được xuất bản như một
  "transport" duy nhất (tức là RouterAddress) trong [NetDB](/en/docs/how/network-database/).
...
- Sử dụng bắt tay 3 thông điệp, một chuyến khứ hồi, như trong [NTCP2](/en/docs/specs/ntcp2/).
  Loại bỏ độ trễ chờ đợi các thông điệp dữ liệu khiến
  [SSU](/en/docs/transport/ssu/) thực tế trở thành bắt tay hai chuyến khứ hồi.
...
- Một transport dựa trên TLS (hoặc giống HTTPS)... đó sẽ là [Proposal 104](/en/proposals/104-tls-transport/).
...
Đề xuất hiện tại cố gắng cung cấp tất cả năm yêu cầu dựa trên giao thức Station-To-Station (STS) giao thức Station-To-Station (STS). Lưu ý rằng giao thức này cũng là cơ sở cho giao thức [SSU](/en/docs/transport/ssu/).
...
* Các mô hình đe dọa: Từ NTCP2 [NTCP2](/en/docs/specs/ntcp2/), với các đe dọa bổ sung đáng kể
...
* Lựa chọn mã hóa: Từ [NTCP2](/en/docs/specs/ntcp2/).

* Handshake: Noise XK từ [NTCP2](/en/docs/specs/ntcp2/) và [NOISE](https://noiseprotocol.org/noise.html). Các đơn giản hóa đáng kể
  cho NTCP2 có thể thực hiện được do tính đóng gói (ranh giới thông điệp vốn có)
  được cung cấp bởi UDP.

* Làm rối khóa tạm thời handshake: Điều chỉnh từ [NTCP2](/en/docs/specs/ntcp2/)
  nhưng sử dụng ChaCha20 từ [ECIES](/en/docs/specs/ecies/) thay vì AES.
...
* Che giấu header gói tin: Được điều chỉnh từ [NTCP2](/en/docs/specs/ntcp2/)
  nhưng sử dụng ChaCha20 từ [ECIES](/en/docs/specs/ecies/) thay vì AES.
...
* Headers được sử dụng làm AEAD associated data như trong [ECIES](/en/docs/specs/ecies/).
...
* Tin nhắn: Được chuyển thể từ [SSU](/en/docs/transport/ssu/)

* I2NP Fragmentation: Chuyển đổi từ [SSU](/en/docs/transport/ssu/)

* Relay và Peer Testing: Chuyển thể từ [SSU](/en/docs/transport/ssu/)

* Chữ ký của dữ liệu Relay và Peer Test: Từ đặc tả cấu trúc chung [Common](/en/docs/specs/common-structures/)

* Định dạng khối: Từ [NTCP2](/en/docs/specs/ntcp2/) và [ECIES](/en/docs/specs/ecies/).

* Padding và options: Từ [NTCP2](/en/docs/specs/ntcp2/) và [ECIES](/en/docs/specs/ecies/).
...
Đề xuất này cung cấp các yêu cầu dựa trên Noise Protocol Framework [NOISE](https://noiseprotocol.org/noise.html) (Revision 33, 2017-10-04). Noise có các đặc tính tương tự như giao thức Station-To-Station (STS), là cơ sở cho giao thức [SSU](/en/docs/transport/ssu/). Trong thuật ngữ của Noise, Alice là bên khởi tạo và Bob là bên phản hồi.
...
Điều này sử dụng cùng định dạng block như đã định nghĩa trong các đặc tả [NTCP2](/en/docs/specs/ntcp2/) và [ECIES](/en/docs/specs/ecies/). Các loại block riêng lẻ được định nghĩa khác nhau. Thuật ngữ tương đương trong QUIC [RFC 9000](https://datatracker.ietf.org/doc/html/rfc9000) là "frames".
...
Có một hoặc nhiều khối trong tải trọng được mã hóa. Một khối có định dạng Tag-Length-Value (TLV) đơn giản. Mỗi khối chứa một định danh một byte, một độ dài hai byte, và không hoặc nhiều byte dữ liệu. Định dạng này giống hệt với định dạng trong [NTCP2](/en/docs/specs/ntcp2/) và [ECIES](/en/docs/specs/ecies/), tuy nhiên các định nghĩa khối thì khác nhau.
...
Cách này sử dụng cùng 9 byte cho header I2NP như trong [NTCP2](/en/docs/specs/ntcp2/) (type, message id, short expiration).
...
Cái này sử dụng cùng 9 byte cho header I2NP như trong [NTCP2](/en/docs/specs/ntcp2/) (type, message id, short expiration).
...
- Độ dài padding sẽ được quyết định dựa trên từng thông điệp cụ thể và
  ...
  của [NTCP2](/en/docs/specs/ntcp2/) cung cấp thêm thông tin về chủ đề này.
...
Tất cả các tính năng của SSU 1 relay đều được hỗ trợ, bao gồm những tính năng được mô tả trong [Proposal 158](/en/proposals/158-ipv6-transport-enhancements/) và được hỗ trợ từ phiên bản 0.9.50. Các introduction IPv4 và IPv6 đều được hỗ trợ. Một Relay Request có thể được gửi qua phiên IPv4 cho một introduction IPv6, và một Relay Request có thể được gửi qua phiên IPv6 cho một introduction IPv4.
...
Các thuộc tính địa chỉ sau đây có thể được công bố, không thay đổi so với SSU 1, bao gồm các thay đổi trong [Đề xuất 158](/en/proposals/158-ipv6-transport-enhancements/) được hỗ trợ từ API 0.9.50:
